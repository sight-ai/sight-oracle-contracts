...
library CapsulatedValueResolver {
    function asCapsulateValue(ebool eb) internal pure returns (CapsulatedValue memory) {
        return CapsulatedValue(ebool.unwrap(eb), Types.T_EBOOL);
    }

    function asCapsulateValue(euint64 eu64) internal pure returns (CapsulatedValue memory) {
        return CapsulatedValue(euint64.unwrap(eu64), Types.T_EUINT64);
    }

    function asCapsulateValue(eaddress eaddr) internal pure returns (CapsulatedValue memory) {
